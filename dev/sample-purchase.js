...
for (let i = 0; i < data.length; i++) {
  ...
  const staffId = purchase.staffId || 1;
  const reviewCode = Math.floor(Math.random() * 2 ** 26).toString(16);

  // insert into sale
  saleInserts.push(
    `INSERT INTO Sale (sale_id, store_id, totalCost, staff_id, review_code) VALUES (${sale_id}, ${store_id}, ${totalCost.toFixed(
      2
    )}, ${staffId}, ${reviewCode});`
  );
  ...
  for (let j = 0; j < purchase.items.length; j++) {
    sale_productInserts.push(
      `INSERT INTO Sale_Product (sale_id, sku_code, quantiy) VALUES (${sale_id}, ${purchase.items[j]}, 1);`
    );
  }
